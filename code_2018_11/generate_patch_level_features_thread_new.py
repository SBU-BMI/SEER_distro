...
if __name__ == '__main__':
  if len(sys.argv)<1:
    print "usage:python generate_patch_level_features_thread_new.py config.json";
    exit();  
  ...
  """
  ext_poly = [(0, 0), (0, 2), (2, 2), (2, 0), (0, 0)]
  int_poly = [(1, 1), (1, 1.5), (1.5, 1.5), (1.5, 1)]
  myPoly = Polygon(ext_poly,[int_poly]);
  myPoly1 = Polygon(ext_poly);
  myPoly2 = Polygon(int_poly);
  print myPoly1.area;
  print myPoly2.area;
  print myPoly.area;
  mult_poly= [
  ...
  polygon = MultiPolygon(mult_poly);    
  print polygon.geom_type;
  exit(); 
  tmp_polygon=[tuple(i) for i in ext_poly];
  ext_polygon = Polygon(tmp_polygon);
  print ext_polygon;
  tmp_polygon2=[tuple(j) for j in int_poly];
  int_polygon = Polygon(tmp_polygon2);
  print int_polygon;
  
  if (int_polygon.within(ext_polygon)): 
    print "within";
  if (ext_polygon.contains(int_polygon)): 
    print "contains"; 
    
  if (ext_polygon.intersects(int_polygon)): 
    print "intersects"; 
  if (ext_polygon.crosses(int_polygon)): 
    print "crosses"; 
  if (ext_polygon.disjoint(int_polygon)): 
    print "disjoint";   
      
  ext_intersect_points =list(zip(*ext_polygon.exterior.coords.xy));
  int_intersect_points =list(zip(*int_polygon.exterior.coords.xy));
  print ext_intersect_points;
  print int_intersect_points;
  newPoly = Polygon(ext_intersect_points,[int_intersect_points]);
  print newPoly;
  exit();
  """   
