...
if __name__ == '__main__':
  ...
  image_array=[];  
  for record in features_histogram.distinct("case_id"):    
  ...
  for case_id in image_array:
    ...
    for record in features_histogram.find({"case_id":case_id,'data_range':'object_level'},{"_id":0,"date":0,"data_range":0}):        
      feature=record["feature"];
      if (feature=='nucleus_area'):
        feature_name="nucleus area (square Micron)"; 
      ...
      plt.title("\n".join(wrap("Object level "+ feature+ ' Histogram of image '+ str(case_id))))
      #Tweak spacing to prevent clipping of ylabel
      plt.subplots_adjust(left=0.15)
      plt.grid(True);           
      # place a text box in upper left in axes coords
      props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
      total_count="{:,}".format(total_object_count)
      textstr="Total object count: " + str(total_count);
