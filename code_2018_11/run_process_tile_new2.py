...
def process_tile(csvfile, jsonfile, svsfile, outfiledir, outtiledir, outtissuedir,pyradiomics_ins): 
    ...
    with open(outfilepath, 'w') as outfile:
        ...
        wrtline = ['case_id', 'image_width', 'image_height', 'mpp_x', 'mpp_y', 'patch_x', 'patch_y', 'patch_width', 'patch_height',
                   'patch_area_micro', 'nuclei_area_micro', 'nuclei_ratio', 'nucleus_material_percentage', 'nuclei_average_area', 'nuclei_average_perimeter', 'has_tumor', 'pseudo-feat'];
        # For pyradiomics
        ...
        for patch_x in xrange(0,tile_width,patch_width_org):
            for patch_y in xrange(0, tile_height, patch_height_org):
                ...
                if nu_ratio>0.0:
                  ...
                  pyrad_feat_fg_tmp=[];
                  pyrad_feat_bg_tmp=[]
                  for feature in feature_name_array:
                    pyrad_feat_fg_tmp.append('n/a');
                    pyrad_feat_bg_tmp.append('n/a');
                  wrtline = wrtline + pyrad_feat_fg_tmp; 
                  wrtline = wrtline + pyrad_feat_bg_tmp; 
                #print pyrad_feat_fg;                
                #print pyrad_feat_bg;
                '''
                ...
                for index,feature_name in enumerate(feature_name_array):
                  ...
                  if nu_ratio>0.0:
                    ...
                    radiomics_json[tmp_name_fg]='n/a';
                    radiomics_json[tmp_name_bg]='n/a';
                  '''  
...
if __name__ == "__main__":    
  if len(sys.argv)<2:
    print "usage:python run_process_tile_new2.py case_id user";
    exit(); 
  ...
  collection_saved= db2.patch_level_radiomics_features_run_3;
  ...
  local_output_folder = os.path.join(my_home, 'SEER_VTR_output_jonas'); 
  ...
  for item  in image_list:  
    ...
    if(len(humanMarkupList_tumor) ==0 and humanMarkupList_non_tumor==0):
      print "No tumor or non tumor regions has been marked in this image by user %s." % user;
      continue;   
    #print "execute to here."  
    
    
    #target_tile="x36864_y34816";    
    ...
    for json_filename in json_filename_list:  
      #if json_filename.find(target_tile) <> -1:#find it!  
      #  print "working on this tile"
      #else:
      #  continue;                        
      ...
      if isTileRelated2TumorRegion(json_file_path,humanMarkupList_tumor,humanMarkupList_non_tumor): 
        ...
        try:    
          ...
          print(e);
          debug_info="image case_id:"+str(case_id)+ " user:"+str(user);
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:    
      ...
      dataset_folder_by_case_id = os.path.join(local_dataset_folder, case_id);
      json_filename_list = [f for f in os.listdir(dataset_folder_by_case_id) if f.endswith('.json')] ; 
      for json_filename in json_filename_list[0:5]:  
        #if json_filename.find(target_tile) <> -1:#find it!  
        #  print "working on this tile"
        #else:
          #continue;                        
        ...
        if isTileRelated2TumorRegion(json_file_path,humanMarkupList_tumor,humanMarkupList_non_tumor): 
          print " --- is processing csv file " + str(csv_file);     
          #executor.submit(process_tile,csv_file_path, json_file_path, image_file, output_folder, output_tile_folder, output_tissue_folder, pyradiomics_ins);
