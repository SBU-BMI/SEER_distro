...
if __name__ == '__main__':
  if len(sys.argv)<2:
    print "usage:python run_object_level_histogram.py case_id user";
    exit();   
  ...
  object_level_histogram = db2.features_histogram; 
  ...
  ###########################################
  def saveHistogramByFeature(feature,nuclear_object_list):
    bin_num=100;
    data_range="object_level";
    hist_count_array=[];
    bin_edges_array=[]; 
    #print "len(nuclear_object_list)";    
    #print len(nuclear_object_list); 
    n, bins, patches = plt.hist(nuclear_object_list,bins=100,facecolor='blue');          
    #plt.xlabel(feature)
  ...
  for item  in image_list:  
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor: 
      for index,tile_min_point in enumerate(unique_tile_min_point_list):  
        ...
        #nuclues_item_list=removeOverlapPolygon(nuclues_item_list) 
      ...
      saveHistogramByFeature(feature,nuclear_object_area_list);
      feature='circularity';      
      saveHistogramByFeature(feature,nuclear_object_circularity_list);
      feature='elongation';      
      saveHistogramByFeature(feature,nuclear_object_elongation_list);      
      feature='grayscale_intensity';      
      saveHistogramByFeature(feature,nuclear_object_grayscale_list);   
