...
if __name__ == "__main__":    
  if len(sys.argv)<0:
    print "usage:python process_tile_in_nfs004.py";
    exit(); 
  
  max_workers=12;  
      
  my_home="/data/shared/bwang";
  ...
  #build image list 
  local_dataset_folder=os.path.join(my_home, 'composite_dataset_merged');
  tumor_image_count=0;
  image_list=[]; 
  for case_id in os.listdir(local_dataset_folder):
    tmp_array=[[],[]];    
    user=getTumorMarkupUser(case_id);    
    if len(user)>0:
      tumor_image_count+=1;
      tmp_array[0]=case_id;   
      tmp_array[1]=user;
  ...
  output_folder0 = os.path.join(my_home, 'patch_level_output'); 
