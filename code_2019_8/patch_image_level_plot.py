...
def generate_Patch_Level_Radiomics_Feature_Plots(feature_array):      
  for csv_file_name in os.listdir(patch_level_csv_folder):
    case_id=csv_file_name.rstrip(".csv");
    #print (case_id); 
    csv_file = os.path.join(patch_level_csv_folder, csv_file_name);
...
def generate_image_level_radiomics_feature_csv_file(feature_array): 
  ...
  elif len(os.listdir(image_level_csv_folder)) ==0:
...
if __name__ == '__main__':   
  ...
  #copy patch level radiomics feature file from nfs003 storage node to local  
  output_path="/data/shared/bwang/radiomics_feature/output";
  patch_level_csv_folder="/home/bwang/patch_level/result/seer_rutger_patch_level_csv/"
  patch_level_plot_folder="/home/bwang/patch_level/result/seer_rutger_patch_level_plot/"
  image_level_csv_folder="/home/bwang/patch_level/result/seer_rutger_image_level_csv/"
  image_level_plot_folder="/home/bwang/patch_level/result/seer_rutger_image_level_plot/"   
  
  #copy patch level radiomics feature csv files from storage node
  if len(os.listdir(patch_level_csv_folder)) == 0: 
    print ("no csv file available, then download from storage node.");
    for sub_folder in os.listdir(output_path):
      print (sub_folder);
      sub_path = os.path.join(output_path, sub_folder);
      csv_file = os.path.join(sub_path, "patch_level_radiomics_features.csv");
      subprocess.call(['scp', csv_file,patch_level_csv_folder]);
      local_file_name=os.path.join(patch_level_csv_folder, "patch_level_radiomics_features.csv");
      new_name=str(sub_folder)+".csv"
      new_file_name=os.path.join(patch_level_csv_folder, new_name);
      shutil.move(local_file_name,new_file_name);       
  else:
     print ("patch level csv files are available.");
