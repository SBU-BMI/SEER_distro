...
def saveFeatures2MongoDB(case_id, image_width, image_height, mpp_x, mpp_y, patch_x, patch_y, patch_width, patch_height,patch_area_micro, nuclei_area_micro, nuclei_ratio, nuclei_average_area, nuclei_average_perimeter,pseud_feat,radiomics_json):
    ...
    for key, value in radiomics_json.items():      
      if value !="None":        
        patch_feature_data[key] = float(value); 
      else:
...
if __name__ == "__main__":    
  if len(sys.argv)<3:
    print ("usage:python run_radiomics_rutger_all_patches.py case_id imageFilePath segmentResultPath");
    exit(); 
  ...
  #setup local folders 
  local_dataset_folder=os.path.join(my_home, 'Rutger_Dataset');
  ...
  local_image_folder = os.path.join(my_home, 'Rutger_Image'); 
  if not os.path.exists(local_image_folder):
    print ('%s folder do not exist, then create it.' % local_image_folder);
    os.makedirs(local_image_folder);  
    
  local_output_folder = os.path.join(my_home, 'Rutger_Output'); 
  ...
  for case_id  in image_list:    
    ...
    tmp_image_name=str(case_id)+".tif";   
    image_file = os.path.join(local_image_folder, tmp_image_name);         
    if not os.path.isfile(image_file):
      print ("image file is not available, then download it from remote server to local folder.");            
      #subprocess.call(['scp', imageFilePath,local_image_folder]);
      try:    
         asyncio.get_event_loop().run_until_complete(run_client(imageFilePath,local_image_folder)) 
