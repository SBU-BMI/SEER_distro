...
if __name__ == '__main__':
  ...
  imageLocation="nfs004:/data/shared/bwang/TCGA_BRCA_images/image/"
  segmentResultLocation="nfs004:/data/shared/bwang/TCGA_BRCA_images/segment_result/";  
  ...
  for case_id in image_list: 
    #print case_id;
    imageFilePath=imageLocation + str(case_id)+".svs"       
    segmentResultPath=segmentResultLocation + str(case_id);      
    #print case_id,imageFilePath,segmentResultPath;    
    cmd = "qsub -l select=1:vnode=^node001 -v case_id=" + case_id + ",imageFilePath="+ imageFilePath +",segmentResultPath=" +segmentResultPath + " run_radiomics_brca_all_patches.pbs";          
    print cmd;
  ...
  if step_num==2:
    #step 2: generate image level or image level radiomics feature dataset
    patch_level_dataset = db.brca_patch_level_radiomics_features;
    collection_saved    = db.brca_image_level_radiomics_features_statistics;     
  ...
  if step_num==3:
    ...
    picture_folder = os.path.join(my_home, 'brca_image_level_plot'); 
    if not os.path.exists(picture_folder):
      print '%s folder do not exist, then create it.' % picture_folder;
      os.makedirs(picture_folder);
    
    patient_level_dataset = db.brca_image_level_radiomics_features_statistics; 
    #patch_level_dataset = db.patch_level_radiomics_features;  
    #image_list=[];
    #for case_id in patch_level_dataset.distinct("case_id"):    
    #  image_list.append(case_id);    
  ...
  if step_num==4:
    ...
    my_home="/home/bwang/patch_level";  
    picture_folder = os.path.join(my_home, 'brca_patch_level_plot'); 
    if not os.path.exists(picture_folder):
      print '%s folder do not exist, then create it.' % picture_folder;
      os.makedirs(picture_folder);
    ...
    #patch_level_dataset = db.patch_level_radiomics_features; 
    patch_level_dataset = db.brca_patch_level_radiomics_features; 
    patch_level_histogram = db.brca_radiomics_features_histogram;
    ...
    name_array=["nuclei_ratio","nuclei_average_area (square micron)","nuclei_average_perimeter (micron)"]; 
  ...
  if step_num==5:
    ...
    my_home="/home/bwang/patch_level";
    csv_folder = os.path.join(my_home, 'brca_patch_level_csv'); 
    if not os.path.exists(csv_folder):
      print '%s folder do not exist, then create it.' % csv_folder;
      os.makedirs(csv_folder);      
    patch_level_dataset = db.brca_patch_level_radiomics_features;     
    
    feature_array=["case_id","image_width","image_height","mpp_x","mpp_y","patch_x","patch_y","patch_width","patch_height","patch_area_micro","nuclei_area_micro","nuclei_ratio","nuclei_average_area","nuclei_average_perimeter"];   
  ...
  if step_num==6:
    #generate image level radiomics feature csv data file 
    my_home="/home/bwang/patch_level";
    csv_folder = os.path.join(my_home, 'brca_image_level_csv'); 
    if not os.path.exists(csv_folder):
      print '%s folder do not exist, then create it.' % csv_folder;
      os.makedirs(csv_folder);  
    ...
    feature_array2=["nuclei_ratio","nuclei_average_area","nuclei_average_perimeter"];   
  ...
  if step_num==7:
    ...
    my_home="/home/bwang/patch_level";  
    picture_folder = os.path.join(my_home, 'brca_image_level_csv'); 
    if not os.path.exists(picture_folder):
      print '%s folder do not exist, then create it.' % picture_folder;
      os.makedirs(picture_folder);      
    ...
    with open(csv_dest_file, 'wb') as csv_write: 
      ...
      for case_id in image_array:    
        ...
        #for record in patch_level_dataset.find({"case_id":case_id} ,{"_id":0}):
        for record in patch_level_dataset.find({"case_id":case_id,"$and":[{"nuclei_ratio":{ "$ne": "None" }},{"nuclei_ratio":{ "$gt": 0.0 }}]}):
