...
if __name__ == "__main__":    
  if len(sys.argv)<1:
    print "usage:python run_radiomics_seer_pdac_all_patches.py case_id";
    exit(); 
  ...
  #remote_host="nfs008"
  #remote_dataset_folder=str(segmentResultPath);
  #remote_image_file = str(remote_host)+":"+str(imageFilePath);
  #remote_dataset_folder=str(remote_host)+":"+str(segmentResultPath);
  #remote_image_file = str(remote_host)+":"+str(imageFilePath);
  ...
  for case_id  in image_list:    
    ...
    user="user"  
    humanMarkupList_tumor,humanMarkupList_non_tumor=findTumor_NonTumorRegions(case_id,user);      
    #humanMarkupList_tumor=findTumor_NonTumorRegions(case_id,user);  
    #if(len(humanMarkupList_tumor) ==0 and humanMarkupList_non_tumor==0): 
    print "len(humanMarkupList_tumor)" + str(len(humanMarkupList_tumor)); 
    print "len(humanMarkupList_non_tumor)" + str(len(humanMarkupList_non_tumor));  
    ...
    for json_filename in json_filename_list:                              
      ...
      if isTileRelated2TumorRegion(json_file_path,humanMarkupList_tumor,""):     
        #print " --- is processing csv file " + str(csv_file);
        try:    
       ...
       print "this tile is not related to tumor region."  ;
