...
def process_tile(csvfile, jsonfile, svsfile, outfiledir, outtiledir, outtissuedir): 
    ...
    with open(outfilepath, 'w') as outfile:
        ...
        wrtline = ['case_id', 'image_width', 'image_height', 'mpp_x', 'mpp_y', 'patch_x', 'patch_y', 'patch_width', 'patch_height',
                   'patch_area_micro', 'nuclei_area_micro', 'nuclei_ratio', 'nuclei_average_area', 'nuclei_average_perimeter', 'has_tumor','pseudo-feat'];
        ...
        for patch_x in xrange(0,tile_width,patch_width_org):
            for patch_y in xrange(0, tile_height, patch_height_org):
                ...
                #print "caseid,img_width, img_height, mpp, mpp, patch_x+tile_x, patch_y+tile_y, patch_width, patch_height,patch_area_micro, nu_area_micro, nu_ratio, nu_area, nu_peri,has_tumor,pseud_feat,radiomics_json";
                #print caseid,img_width, img_height, mpp, mpp, patch_x+tile_x, patch_y+tile_y, patch_width, patch_height,patch_area_micro, nu_area_micro, nu_ratio, nu_area, nu_peri,has_tumor,pseud_feat,radiomics_json;               
                saveFeatures2MongoDB(caseid,img_width, img_height, mpp, mpp, patch_x+tile_x, patch_y+tile_y, patch_width, patch_height,patch_area_micro, nu_area_micro, nu_ratio, nu_area, nu_peri,has_tumor,pseud_feat,radiomics_json); 
##############################################################################################
...
#############################################################################  
def saveFeatures2MongoDB(case_id, image_width, image_height, mpp_x, mpp_y, patch_x, patch_y, patch_width, patch_height,patch_area_micro, nuclei_area_micro, nuclei_ratio, nuclei_average_area, nuclei_average_perimeter,has_tumor,pseud_feat,radiomics_json):
...
#############################################################################  
def saveFeatures2MongoDB_old(case_id, image_width, image_height, mpp_x, mpp_y, patch_x, patch_y, patch_width, patch_height,patch_area_micro, nuclei_area_micro, nuclei_ratio, nuclei_average_area, nuclei_average_perimeter,pseud_feat):
    patch_feature_data = collections.OrderedDict();
    patch_feature_data['case_id'] = case_id;
    patch_feature_data['image_width'] = image_width;
    patch_feature_data['image_height'] = image_height;  
...
if __name__ == "__main__":    
  if len(sys.argv)<0:
    print "usage:python compute_radiomics_all_patches.py";
    exit(); 
    
  #get case_id and user from prompt input
  #case_id= sys.argv[1];
  #imageFilePath=sys.argv[2];
  #segmentResultPath= sys.argv[3]; 
  
  case_id="VTRPDAC_Test_PC3896_BL1_XX" ;
  imageFilePath="/home/bwang/patch_level/SEER_PDAC_image/VTRPDAC_Test_PC3896_BL1_XX.svs";
  segmentResultPath="/home/bwang/patch_level/SEER_PDAC_dataset/VTRPDAC_Test_PC3896_BL1_XX/" ;
  ...
  config_json_file_name = "config_brca.json";  
  ...
  db = client[db_name];        
  collection_saved= db.test_patch_level_radiomics_features;
  
  #setup local folders 
  local_dataset_folder=os.path.join(my_home, 'SEER_PDAC_dataset');
  ...
  local_image_folder = os.path.join(my_home, 'SEER_PDAC_image'); 
  if not os.path.exists(local_image_folder):
    print '%s folder do not exist, then create it.' % local_image_folder;
    os.makedirs(local_image_folder);  
    
  local_output_folder = os.path.join(my_home, 'SEER_PDAC_output'); 
  ...
  for case_id  in image_list:    
    ...
    if os.path.isdir(local_dataset_folder_detail) and len(os.listdir(local_dataset_folder_detail)) > 0: 
      ...
      subprocess.call(['scp', segmentResultPath+'/*.json',local_dataset_folder_detail]);
      subprocess.call(['scp', segmentResultPath+'/*features.csv',local_dataset_folder_detail]);       
  
    #copy image svs file from remote folder to local folder  
    tmp_image_name=str(case_id)+".svs";   
    image_file = os.path.join(local_image_folder, tmp_image_name);         
    if not os.path.isfile(image_file):
      print "image svs file is not available, then download it to local folder.";            
      subprocess.call(['scp', imageFilePath,local_image_folder]);   
    else:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:        
      ...
      for json_filename in json_filename_list[0:100]:                              
