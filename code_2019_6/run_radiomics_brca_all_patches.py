    ...
    with open(outfilepath, 'w') as outfile:
        ...
        for patch_x in xrange(0,tile_width,patch_width_org):
            for patch_y in xrange(0, tile_height, patch_height_org):
                ...
                saveFeatures2MongoDB(caseid,img_width, img_height, mpp, mpp, patch_x+tile_x, patch_y+tile_y, patch_width, patch_height,patch_area_micro, nu_area_micro, nu_ratio, nu_area, nu_peri,pseud_feat);  
##############################################################################################
...
#############################################################################  
def saveFeatures2MongoDB_copy(case_id, image_width, image_height, mpp_x, mpp_y, patch_x, patch_y, patch_width, patch_height,patch_area_micro, nuclei_area_micro, nuclei_ratio, nuclei_average_area, nuclei_average_perimeter,pseud_feat,radiomics_json):
...
#############################################################################  
def saveFeatures2MongoDB(case_id, image_width, image_height, mpp_x, mpp_y, patch_x, patch_y, patch_width, patch_height,patch_area_micro, nuclei_area_micro, nuclei_ratio, nuclei_average_area, nuclei_average_perimeter,pseud_feat):
    patch_feature_data = collections.OrderedDict();
    patch_feature_data['case_id'] = case_id;
    patch_feature_data['image_width'] = image_width;
    patch_feature_data['image_height'] = image_height;  
...
if __name__ == "__main__":    
  if len(sys.argv)<3:
    print "usage:python run_radiomics_brca_all_patches.py case_id imageFilePath segmentResultPath";
    exit(); 
  ...
  db = client[db_name];        
  collection_saved= db.brca_patch_level_radiomics_features;
  
  #setup local folders 
  local_dataset_folder=os.path.join(my_home, 'BRCA_dataset');
  ...
  local_image_folder = os.path.join(my_home, 'BRCA_image'); 
  if not os.path.exists(local_image_folder):
    print '%s folder do not exist, then create it.' % local_image_folder;
    os.makedirs(local_image_folder);  
    
  local_output_folder = os.path.join(my_home, 'BRCA_output'); 
